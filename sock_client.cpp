...
mysock::Client::Client(const char *server_address, int port)
{
    ...
    if (err != 0)
        throw mysock::exception<int>(err,"WSAStartup error");
    ...
    {
        WSACleanup();
        throw mysock::exception<std::string>("LOBYTE and HIBYTE error");
    }
#ifdef _MSC_VER
    char **pptr = nullptr;
    ...
    if (nullptr != pHostEntry && server_address[0] != '\0')
    {
        ...
        throw mysock::exception<std::string>("get host name fail");
#endif //_MSC_VER
    ...
    // 创建客户端套节字
    IN_ADDR tmp;
    tmp.s_addr = inet_addr(Real_Addr.c_str());
    Socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    Socket_info = {PF_INET, htons(port), tmp};
...
bool mysock::Client::connect2server()
{
